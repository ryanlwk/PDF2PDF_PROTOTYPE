...
def step3_workspace():
    ...
    with col_original:
        st.subheader("原始文件 (Original)")
        
        # Fix file pointer before reading
        if (hasattr(st.session_state, 'uploaded_file') and 
            st.session_state.uploaded_file is not None and 
            hasattr(st.session_state.uploaded_file, 'read')):
            uploaded_file = st.session_state.uploaded_file
            uploaded_file.seek(0)  # Reset pointer before reading
            
            # Use a fixed-height container to enable scrolling (match middle column height)
            with st.container(height=pdf_height):
                doc = None
                try:
                    ...
                    for page_num in range(len(doc)):
                        ...
                        # Use 2x matrix for quality (matches display_pdf function)
                        pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))
                        # width="stretch" fills the container
                        st.image(pix.tobytes("png"), caption=f"Page {page_num + 1} of {len(doc)}", width="stretch")
                except Exception as e:
                    st.error(f"Error: {e}")
                finally:
                    if doc:
            ...
            uploaded_file.seek(0)  # Reset pointer for downstream tasks
        else:
            # Fallback if uploaded_file is not available
            st.warning("⚠️ Uploaded file not available")
