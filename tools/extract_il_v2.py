...
def extract_pdf_style_aware(input_pdf: str = None):
    """
    ...
    Output: intermediate_layer.json with style metadata
    
    Args:
        input_pdf: Path to input PDF file (default: "somatosensory.pdf")
    """
    if input_pdf is None:
        input_pdf = "somatosensory.pdf"
    output_file = "intermediate_layer.json"
    ...
    for btype, count in sorted(all_types.items()):
        print(f"   • {btype}: {count}")
    print()
    print("👉 Next step: python tools/translate_il.py")
    print()

if __name__ == "__main__":
    import sys
    input_pdf = sys.argv[1] if len(sys.argv) > 1 else None
    extract_pdf_style_aware(input_pdf)
